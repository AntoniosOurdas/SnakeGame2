...
public class Board {
	...
	public void createBoard() {
		...
		// flag is used as an indicator when to fill every row in ascending or
		// descending order
		...
		for (int i = 0; i < snakes.length; ++i) {
			...
			while (snakeExists || tailAboveHead) {
				...
				// Create a new snake
				snakes[i] = new Snake(snakesId, (int) (Math.random() * (N * M - 1) + 1),
						(int) (Math.random() * (N * M - 1) + 1));
				...
				for (int j = 0; j < snakesInitialized; ++j) {
					if (snakes[j].getHeadId() == snakes[i].getHeadId()
							|| snakes[j].getTailId() == snakes[i].getTailId()) {
						snakeExists = true;
						break;
					}
				}
		...
		for (int i = 0; i < ladders.length; ++i) {
			...
			while (ladderExists || upStepAboveDownStep || ladderSnakeOverlap) {
				...
				// Create a new ladder
				ladders[i] = new Ladder(laddersId, (int) (Math.random() * (N * M - 1) + 1),
						(int) (Math.random() * (N * M - 1) + 1), false);

				// Check if the downStep of ladder is above upStep (if not then
				...
				for (int j = 0; j < laddersInitialized; ++j) {
					if (ladders[j].getUpStepId() == ladders[i].getUpStepId()
							|| ladders[j].getDownStepId() == ladders[i].getDownStepId()) {
						ladderExists = true;
						break;
					}
				}
	...
	public void createElementBoard() {
		...
		// An efficient way to do so is to develop an algorithm to give us directly the
		// position (x, y)
		// of an element (snake head or tail, ladder top or base, apple) depended on its
		// id
		...
		for (int i = 0; i < snakes.length; ++i) {
			...
			// we find how many times does the id of element contain M (i.e. take the
			// division id / M)
			// But because the last element of the row will take the value (y + 1) / M
			// the result of the division result will be y + 1 (greater than the actual
			// distance y)
			//
			...
			// y = (17 - 1) / 5 = 16 / 5 = 3
			// This means the element is 3 rows above the 4th row, hence it belongs on the 4
			// - 3 = 1st row
			// Also its distance from the 4th row is 3 which is odd
			//
			// Hence, the elements are filled in in descending order
			// So it's distance from the 5th column is 1 (the remainder from the division
			// ((17 - 1) % 5))
			// To find the column in which it belongs, we have to find the complement of its
			// distance
			...
			// Here we make the division (id - 1) % M and not id % M for the same reason as
			// in row search
